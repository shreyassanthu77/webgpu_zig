...
pub fn run(a: std.mem.Allocator, s: Schema, out: *std.Io.Writer) !void {
    ...
    for (s.objects) |obj| {
        ...
        if (obj.methods) |methods| for (methods) |m| {
            try emitFnTest(&w, op, naming.cSymbol(a, &.{ obj.name, m.name }));
        };
        if (obj.extended != true) {
            try emitFnTest(&w, op, naming.cSymbol(a, &.{ obj.name, "add_ref" }));
            try emitFnTest(&w, op, naming.cSymbol(a, &.{ obj.name, "release" }));
        }
    }
    for (s.structs) |st| {
        if (st.free_members == true)
            try emitFnTest(&w, naming.pascal(a, st.name), naming.cSymbol(a, &.{ st.name, "free_members" }));
    }
    for (s.functions) |f| {
        const c_name = naming.cSymbol(a, &.{f.name});
        try w.line("test \"abi fn {s}\" {{", .{c_name});
        try w.line("    try test_helpers.expectFnAbi(@TypeOf({s}), @TypeOf(c.{s}));", .{ c_name, c_name });
        try w.line("}}", .{});
    }
...
fn emitFnTest(w: *Writer, container: []const u8, c_name: []const u8) !void {
    try w.line("test \"abi fn {s}\" {{", .{c_name});
    try w.line("    try test_helpers.expectFnAbi(@TypeOf({s}.{s}), @TypeOf(c.{s}));", .{ container, c_name, c_name });
    try w.line("}}", .{});
}
